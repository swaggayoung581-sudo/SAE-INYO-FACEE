...
do
    ...
    -- While Auto Steal is active, keep jumping disabled. The game can replace
    -- the Humanoid during a steal, so the watcher reapplies the lock to the
    -- current Humanoid and restores the normal jump state when stealing stops.
    local jumpLockActive = false
    local savedJumpPower, savedJumpHeight, savedJumpEnabled

    local function setStealJumpLock(on)
        jumpLockActive = on and true or false
        ...
        if hum then
            if on then
                ...
                savedJumpEnabled = hum:GetStateEnabled(Enum.HumanoidStateType.Jumping)
                hum.JumpPower = 0
                hum.JumpHeight = 0
                ...
                hum:SetStateEnabled(Enum.HumanoidStateType.Jumping, savedJumpEnabled ~= false)
                hum.Jump = false
            end
        end
    end
    ...
    local mainAutoStealButton, setMainAutoVisual = toggle("Auto Steal", function(on)
        ...
        if ok == false then
            setStealJumpLock(false)
            error(why or "Could not change Auto Steal")
        end
        setStealJumpLock(on)
    ...
    autoStealButton.Text = "🏃  AUTO STEAL"
    ...
    autoStealButton.MouseButton1Click:Connect(function()
        if not autoStealOn then
            -- Use the exact same Auto Steal engine/options as Menu 1.
            -- If there is no selection, Auto Find chooses one first.
            ...
            setStealJumpLock(true)
            setSecondAutoVisual(true)
            syncMainAutoVisual(true)
            findStatus.Text = "AUTO STEAL: ON • JUMP LOCKED"
        else
